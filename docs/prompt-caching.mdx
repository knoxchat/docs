...
When using caching (whether automatically enabled in supported models or via the `cache_control` header), Knox will make every effort to continue routing to the same provider to take advantage of warm caches. If the provider that cached the prompt becomes unavailable, Knox will attempt the next best provider.
