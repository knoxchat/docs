...
Direct `/v1/realtime` clients get the same injection on `session.update` (and a fallback update after `session.created` if they never send one). Knox Chat microphone sessions do this automatically.
