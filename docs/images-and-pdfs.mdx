...
Knox supports sending images and PDF files via the API. This article will show you how to use our API to handle these two file types.
...
Knox provides PDF processing capabilities through the `/v1/chat/completions` API. PDF files can be sent as base64-encoded data URLs in the message array via the file content type. This feature is available for **any** model on Knox.

:::info
  If the model natively supports file input, the PDF will be passed directly to the model. If the model does not natively support file input, Knox will parse the file and pass the parsed results to the requested model.
:::
...
To configure PDF processing functionality, use the `plugins` parameter in the request. Knox offers multiple PDF processing engines with varying features and pricing:
...
Knox offers multiple PDF processing engines:
...
If no engine is explicitly specified, Knox will prioritize the model's native file handling capability. If unavailable, it defaults to the `"mistral-ocr"` engine.  
...
:::info
  When you include file comments from previous responses in subsequent requests, Knox will directly use this pre-parsed information  
