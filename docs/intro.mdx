...
<div className="bg-white dark:bg-transparent rounded-md border border-slate-200 dark:border-slate-700 shadow-sm my-4">
  <Accordion type="single" collapsible className="w-full">
    ...
    <AccordionItem value="item-3">
      ...
      <AccordionContent>
        Knox Chat implements the OpenAI API specification for `/completions` and `/chat/completions`, allowing you to use any model with the same request/response format. Additional endpoints like `/v1/models`, `/v1/realtime`, `/v1/videos`, and Knox‑MS routes are also available. See our [API Reference](/1.0.0/api-reference) for detailed specifications.
      </AccordionContent>
    </AccordionItem>
    <AccordionItem value="item-4">
