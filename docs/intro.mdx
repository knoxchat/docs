...
**Knox** is a unified, OpenAI-compatible API, Anthropic Messages API format supporting and platform that gives you access to hundreds of models through one endpoint, with intelligent routing, fallbacks, and transparent pricing. It pairs that with **Knox‑MS**, a memory‑centric system for long‑running agents and apps that need persistent context.

### What you get

- **Smart model routing**: performance/cost/balanced strategies with automatic fallback and provider metrics.
- **Multimodal I/O**: text, images, PDFs, and audio inputs, plus image, video, and [music](/music) generation, [text to speech](/text-to-speech), [speech to text](/speech-to-text), [voices](/voices), [voice changer](/voice-changer), [Voice Isolator](/voice-isolator), [sound effects](/sound-effects), [dubbing](/dubbing), [forced alignment](/forced-alignment), and speech-to-speech [Realtime](/realtime).
...
- **Jev (System One)**: typed questions against a shared state, structured answers (not an LLM). See [Jev](/jev).
...
Knox‑MS adds multi‑level memory, summaries, vector search, and a knowledge graph to power persistent agents and workflows. It also supports autonomous planning, task orchestration, self‑healing, and realtime progress events. Learn more in the <Link to="/blog/kms-docs">Knox Memory System — Full Feature Deep Dive</Link>.
...
For information about using third-party SDKs and frameworks with Knox, please see our frameworks documentation.

## Principles

Knox helps teams build reliable, cost‑efficient AI systems across providers. We believe the future is multimodal, multi‑provider, and memory‑centric.

### Why Knox?

Price and Performance. Knox scouts for the best prices, lowest latencies, and highest throughput across providers, and lets you choose how to prioritize them.
...
Multimodal by Default. Images, PDFs, and audio inputs plus image generation, video generation, and speech-to-speech Realtime make multimodal workflows straightforward.
...
<div className="bg-white dark:bg-transparent rounded-md border border-slate-200 dark:border-slate-700 shadow-sm my-4">
  <Accordion type="single" collapsible className="w-full">
    <AccordionItem value="item-1">
      <AccordionTrigger>Why should I use Knox?</AccordionTrigger>
      <AccordionContent>
        Knox provides a unified API that gives you access to hundreds of AI models through a single endpoint, with intelligent routing, automatic fallback, and transparent pricing. It also includes **Knox‑MS**, a memory system for persistent agents and workflows. Our ["Activity"](https://knox.chat/activity) and ["Stats"](https://knox.chat/stats) pages let you analyze usage, cost, and performance.
      </AccordionContent>
    </AccordionItem>
    <AccordionItem value="item-2">
      <AccordionTrigger>How do I get started with Knox?</AccordionTrigger>
        <AccordionContent>
          Getting started with **Knox** is simple. Register at [**register**](https://knox.chat/register) (or use [**GitHub**](https://github.com) to [**Login**](https://knox.chat/login)), then create an API key on the [**Keys**](https://knox.chat/keys) page. Pick models from the [**Models**](https://knox.chat/models) page and start calling the OpenAI‑compatible API. If you want persistent memory, enable Knox‑MS in your settings or use the Knox‑MS endpoints.
        </AccordionContent>
      </AccordionItem>
    ...
    <AccordionItem value="item-3b">
      <AccordionTrigger>What is Jev and when should I use it?</AccordionTrigger>
        <AccordionContent>
          Jev is TypeSafe's System One model on Knox. It is not an LLM: you send a `state` and typed questions (yes/no, choice, or score) to [`POST /v1/systemone`](/1.0.0/systemone) and get structured answers your code can use directly. Use it for routing, classification, and calibrated judgments. Use chat completions when you need generated text. See the [Jev](/jev) guide.
        </AccordionContent>
    </AccordionItem>
    ...
    <AccordionItem value="item-5">
      <AccordionTrigger>How do I pay for using Knox?</AccordionTrigger>
      <AccordionContent>
        ...
        When you make a request to **Knox**, we use provider‑reported usage to calculate fees and deduct them from your balance on the [**Credits**](https://knox.chat/credits) page. Requests covered by zero‑completion insurance (empty/failed generations) are not billed. Full usage records are available on the [**Activity**](https://knox.chat/activity) page.
      </AccordionContent>
    </AccordionItem>
    <AccordionItem value="item-6">
      <AccordionTrigger>How do I recharge?</AccordionTrigger>
      <AccordionContent>
        Recharge can be done through the [**Credits**](https://knox.chat/credits) page to make payments and recharge for using **[Knox](https://knox.chat)**.
      </AccordionContent>
    </AccordionItem>
  </Accordion>
</div>
...
<div className="bg-white dark:bg-transparent rounded-md border border-slate-200 dark:border-slate-700 shadow-sm my-4">
  <Accordion type="single" collapsible className="w-full">
    <AccordionItem value="item-1">
      <AccordionTrigger>What LLM models does Knox support?</AccordionTrigger>
      <AccordionContent>
        Knox provides access to a wide variety of LLMs, including frontier models from major providers, plus multimodal and image‑generation models. For a complete list, visit the [Models List](https://knox.chat/models) or fetch the list through the [models api](https://api.knox.chat/v1/models).
      </AccordionContent>
    </AccordionItem>
    <AccordionItem value="item-2">
    ...
    <AccordionItem value="item-3">
      ...
      <AccordionContent>
        ...
        2. `:beta` - The model is not moderated by Knox.
    ...
    <AccordionItem value="item-5">
      ...
      <AccordionContent>
        For each model, Knox shows latency (time to first token) and throughput across providers. You can optimize routing with `:nitro` for throughput, `:floor` for cost, or set routing preferences for performance and reliability.
      </AccordionContent>
    </AccordionItem>
    <AccordionItem value="item-6">
      <AccordionTrigger>How does model fallback work if a provider is unavailable?</AccordionTrigger>
      <AccordionContent>
        If a provider returns an error, Knox automatically falls back to the next best provider. This is transparent to the user and improves resilience. You can configure routing strategies, weights, and fallback behavior in the [Model Routing](/model-routing) documentation.
      </AccordionContent>
    </AccordionItem>
  </Accordion>
</div>
...
<div className="bg-white dark:bg-transparent rounded-md border border-slate-200 dark:border-slate-700 shadow-sm my-4">
  <Accordion type="single" collapsible className="w-full">
    <AccordionItem value="item-1">
      ...
      <AccordionContent>
        Knox uses two authentication methods:
    ...
    <AccordionItem value="item-3">
      ...
      <AccordionContent>
        Knox implements the OpenAI API specification for `/completions` and `/chat/completions`, allowing you to use any model with the same request/response format. Additional endpoints like `/v1/models`, `/v1/systemone` ([Jev](/jev)), `/v1/realtime`, `/v1/videos`, and Knox‑MS routes are also available. See our [API Reference](/1.0.0/api-reference) for detailed specifications.
      </AccordionContent>
    </AccordionItem>
    <AccordionItem value="item-4">
      <AccordionTrigger>What are the supported formats?</AccordionTrigger>
      <AccordionContent>
        The API supports text, PDFs, images, audio, video generation, and speech-to-speech Realtime. Images can be passed as URLs or base64 data URLs. Chat audio must be base64 encoded. Image generation outputs are supported for models with image output modalities. Video generation uses the asynchronous `/v1/videos` API — see [Video Generation](/video-generation). Live voice agents use `/v1/realtime` — see [Realtime](/realtime).
      </AccordionContent>
    </AccordionItem>
    <AccordionItem value="item-5">
    ...
    <AccordionItem value="item-6">
      ...
      <AccordionContent>
        Knox is a drop‑in replacement for OpenAI, so any OpenAI‑compatible SDKs and frameworks work out of the box, including tool calling, structured outputs, and reasoning parameters.
      </AccordionContent>
    </AccordionItem>
  </Accordion>
</div>
...
<div className="bg-white dark:bg-transparent rounded-md border border-slate-200 dark:border-slate-700 shadow-sm my-4">
  <Accordion type="single" collapsible className="w-full">
    ...
    <AccordionItem value="item-2">
      ...
      <AccordionContent>
        Knox is a proxy system that sends your requests to model providers to fulfill them. We work with all providers to ensure that prompts and generated outputs are not logged or used for training whenever possible.
      </AccordionContent>
    </AccordionItem>
    <AccordionItem value="item-3">
