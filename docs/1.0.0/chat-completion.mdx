...
| transforms | List of Strings | No | A list of prompt transformations (Knox only). |
| web_search | Boolean | No | Enable or disable automatic web search for Anthropic models. Defaults to `true` for Anthropic models (e.g. `anthropic/claude-sonnet-4.6`). Set to `false` to disable. No effect on non-Anthropic models. |
...
### With Web Search (Anthropic Models)

Anthropic models automatically have web search enabled. The model will search the web when the query requires up-to-date information. To disable web search:
...
> **Note:** Web search is billed at $10 per 1,000 search calls in addition to standard token costs. Each search invocation by the model counts as one call.
