...
Returns the public catalog of video generation models available through Knox Chat. Use this list to choose a `model` ID and to constrain `aspect_ratio`, `resolution`, `size`, `duration`, `seed`, `generate_audio`, and `frame_images` before calling [`POST /v1/videos`](/1.0.0/create-video).
