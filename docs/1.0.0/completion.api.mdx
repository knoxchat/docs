...
| transforms | List of Strings | No | List of prompt transformations (Knox only). |
| web_search | Boolean | No | Enable or disable automatic web search for Anthropic models. Defaults to `true` for Anthropic models. Set to `false` to disable. No effect on non-Anthropic models. |
