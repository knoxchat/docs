...
For dedicated speech APIs, use [Text to Speech](/text-to-speech) (`POST https://api.knoxstudio.ai/v1/text-to-speech/{voice_id}`) and [Speech to Text](/speech-to-text) (`POST https://api.knoxstudio.ai/v1/speech-to-text`). For live speech-to-speech (microphone in, audio out) with `gpt-realtime-2.1` or `gpt-realtime-2.1-mini`, use the [Realtime](/realtime) API instead of `/v1/chat/completions`.
