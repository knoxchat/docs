...
**Knox** supports sending audio files to compatible models via the API. This guide will show you how to work with audio using our API.

**Note**: Audio files must be **base64-encoded** - direct URLs are not supported for audio content.

For dedicated speech APIs, use [Text to Speech](/text-to-speech), [Speech to Text](/speech-to-text), [Voices](/voices), [Voice Changer](/voice-changer), [Voice Isolator](/voice-isolator), [Sound Effects](/sound-effects), [Dubbing](/dubbing), and [Forced Alignment](/forced-alignment). For live speech-to-speech (microphone in, audio out) with `gpt-realtime-2.1` or `gpt-realtime-2.1-mini`, use the [Realtime](/realtime) API instead of `/v1/chat/completions`.
