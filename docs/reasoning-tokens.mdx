...
For models supporting this API, the Knox API can return **Reasoning Tokens**, also known as reasoning tokens. Knox standardizes the different ways custom models use reasoning tokens, providing a unified interface for different providers.
...
For backward compatibility, Knox still supports the following legacy parameters:
...
The latest Claude models, such as `anthropic/claude-sonnet-4.6`, support the use and return of inference tokens.

You can enable the inference feature for Anthropic models in two ways:

1. Use the `:thinking` variant suffix (e.g., `anthropic/claude-sonnet-4.6:thinking`). This variant enables high-intensity inference (`"effort": "high"`) by default.
