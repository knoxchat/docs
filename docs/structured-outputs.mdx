...
Knox supports structured outputs for compatible models, ensuring that responses adhere to a specific JSON Schema format. This feature is particularly useful when you need consistent, well-formatted responses that can be reliably parsed by your application.

If you need a **decision** (yes/no, pick one option, score a rubric) rather than generated JSON from an LLM, use [Jev](/jev) (`POST /v1/systemone`) instead.
...
* OpenAI models (GPT-5.2 and above) [Documentation](https://platform.openai.com/docs/guides/structured-outputs)  
