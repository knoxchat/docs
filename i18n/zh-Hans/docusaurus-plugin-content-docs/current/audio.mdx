...
专用语音接口请使用 [文本转语音](/text-to-speech)（`POST https://api.knoxstudio.ai/v1/text-to-speech/{voice_id}`）和 [语音转文本](/speech-to-text)（`POST https://api.knoxstudio.ai/v1/speech-to-text`）。如需实时语音到语音（麦克风输入、音频输出），请使用 `gpt-realtime-2.1` 或 `gpt-realtime-2.1-mini` 的 [实时语音](/realtime) API，而不是 `/v1/chat/completions`。
