...
专用语音接口请使用 [文本转语音](/text-to-speech)、[语音转文本](/speech-to-text)、[音色](/voices)、[语音转换](/voice-changer)、[人声分离](/voice-isolator)、[音效生成](/sound-effects)、[配音](/dubbing) 和 [强制对齐](/forced-alignment)。如需实时语音到语音（麦克风输入、音频输出），请使用 `gpt-realtime-2.1` 或 `gpt-realtime-2.1-mini` 的 [实时语音](/realtime) API，而不是 `/v1/chat/completions`。
