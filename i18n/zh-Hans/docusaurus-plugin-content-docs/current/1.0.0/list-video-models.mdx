...
返回 Knox Chat 上可用的公开视频生成模型目录。使用该列表选择 `model` ID，并在调用 [`POST /v1/videos`](/1.0.0/create-video) 之前约束 `aspect_ratio`、`resolution`、`size`、`duration`、`seed`、`generate_audio` 和 `frame_images`。
