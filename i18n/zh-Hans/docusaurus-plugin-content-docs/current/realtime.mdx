...
您也可以在 Knox Chat 界面中使用 **OpenAI: GPT Realtime 2.1** 或 **OpenAI: GPT Realtime 2.1 Mini**：选择该模型、选择 API key，然后点击麦克风。
...
直接连接 `/v1/realtime` 的客户端会在 `session.update` 时获得同样的注入（若从未发送 `session.update`，会在 `session.created` 之后做一次兜底更新）。Knox Chat 麦克风会话会自动执行此流程。
