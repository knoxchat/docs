...
通过将 MCP (Anthropic) 工具定义转换为 OpenAI 兼容的工具定义，你可以在 Knox Chat 中使用 MCP 服务器。

在本示例中，我们将使用 [Anthropic MCP 客户端 SDK](https://github.com/modelcontextprotocol/python-sdk?tab=readme-ov-file#writing-mcp-clients) 与文件系统 MCP 交互，全部通过 Knox Chat 在后台完成。
